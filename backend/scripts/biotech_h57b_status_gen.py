...
def main():
    ...
    lines += [
        ...
        "---",
        "",
        "## 사용자가 하실 일 (자동화 이후 · WP69-3h 완결)",
        "",
        "**자동 실행**: 서버 optimus8 이 매일 KST 07:00 (일일 파이프) · 월요일 06:00 (AACT 주간) · 15일 08:00 (60일 전향 평가) 자동 실행. **로컬 crontab 불필요**.",
        "",
        "- [ ] **웹 열람 (일상)**: `https://optimus8.cafe24.com/biotech` · admin 토큰 로그인 · 소문 탭·순위표 탭·글 탭 확인",
        "- [ ] **실전 기록 (선택 · 주 1회 git push)**: `docs/plans/biotech/data/trades_manual.csv` 편집 → `git push` → 서버 전향 평가가 자동으로 읽음",
        "  - 열: `ticker,side,qty,price,timestamp_utc,status,note` (status = OPEN/CLOSED)",
        "  - 없어도 무방 (없으면 '실전 기록 없음' · 가상 규칙만 평가)",
        "- [ ] **알림 확인 (실패 시만)**: 텔레그램 봇이 실패 단계·원인 발송 · 성공 시 무알림",
        "- [ ] **월간 회고 (매월 15일 이후)**: `var/biotech/verification/forward/forward-60d-*.md` 확인 (또는 서버에서 git pull 되면 docs 하위에 복사)",
        "",
        "---",
        "",
        manual_map.rstrip(),
