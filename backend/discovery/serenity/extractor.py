...
async def load_pending_tweets(limit: int = 50) -> list[SerenityTweet]:
    """SerenitySignal 이 없는 트윗 최근순 조회.

    LEFT JOIN 대안: EXISTS subquery (SQLAlchemy 안전 · SQLite/PG 공통).
    """
    async with get_session() as session:
        signal_exists = (
            select(SerenitySignal.tweet_id)
            .where(SerenitySignal.tweet_id == SerenityTweet.tweet_id)
            .exists()
        )
        stmt = (
            select(SerenityTweet)
            .where(not_(signal_exists))
...
async def process_pending_tweets(
    ...
    *,
    ...
    """미처리 트윗 배치 z.ai 추출 → serenity_signals upsert.

    반환: {"tweets": N, "signals_inserted": M, "failed": K}
    """
    pending = await load_pending_tweets(limit=batch_size)
    if not pending:
        return {"tweets": 0, "signals_inserted": 0, "failed": 0}
