...
**작성일**: 2026-07-22 · **개정 2026-07-27** (v1.48 세션 종료 시점 · v3.0)
**대상**: 다음 세션 재개 시 · Claude 및 사용자
**현 라이브**: **v1.48 · `c796e92`** (12개 배포 완결 · 화약고 v2 조건 확정 · 정체성 v2.0 · 활용 단계)

## ⚡ 사용자 활용 (2026-07-27 최종)

**정체성** (identity.md v2.0): 2026 하반기 한국주식 투자 이익 창출 · 다른 투자자보다 정확한 정보로 매수/매도 판단.

**현 Tier 1 lock 10 종목**: 세원물산·경동도시가스·오리콤·SNT홀딩스·SJM·신라교역·비츠로테크·경동인베스트·삼양케이씨아이·넵튠 (경인전자는 v2 규모 미달 강등)

**실 사용법**:
1. Telegram 알림 (3분 자동 폴링) → 이벤트 감지
2. `optimus8.cafe24.com/powderkeg` 접속 · 종목명 클릭 → 상세 팝업
3. "🔄 변동 이력 + Provenance" 탭에서 조건별 근거 확인
4. 외부 링크 (KRX·네이버·다음·DART) 크로스 확인
5. 반자동 티켓 생성 (validated 이벤트만)
...
### ✅ 완결 · 배포 완료 (2026-07-22 ~ 07-27 세션)
- **P4-1** provenance UI + Run diff (v1.38 · `3f7edb2`)
- **P4-6** 발굴 조건 분리 표기 (v1.38)
- **P4-5** KRX 관리종목 실데이터 (v1.39 · `49d0922`)
- **P2-1** 상폐 재무 백필 (v1.40 · `6b22962`) · 322 rows / 136 tickers
- **P2-2** PIT 층화 백테스트 (v1.41 · `3c62d20`) + `/backtest/stratified` 라우트 (v1.42 · `8314688`)
- **P2-1b** 활성 종목 재무 백필 (v1.43 · `1e37134`) · +4,232 rows (2020-2022)
- **P2-2b** Grid Search API (v1.44 · `8bec550`) · 화약고 조건 완화 실측 → 시나리오 B 확증
- **P2-2c** 역설계 CAR 상위·하위 특성 대조 (v1.45 · `eff42a8`) · identity.md v2.0 정체성 명시
- **P2-2d** 화약고 v2 조건 · F-Score 4 완화 + 규모 필터 (v1.46 · `1b77a29`)
- **P2-2d hotfix** 규모 임계 완화 (v1.47 · `9510f69`) · 세원·넵튠 Tier 1 복원
- **P2-2d v2 정합** pit_evaluate config 자동 반영 + 조건 11 (v1.48 · `c796e92`)
...
### 🎯 남은 우선순위 (다음 세션 · 사용자 활용 정착 후)

**우선순위 1 · Phase 2 완전 PIT 준비** (자연 축적 3개월)
- KrxMarketSnapshot 매일 append 스케줄러 (현 2일치)
- PowderKegKrxIssue 매일 append (P4-5 refresh API 스케줄러화)
- 축적 후 시장 조건(1_pbr·9_adv60)·관리 조건(10) as-of 평가 가능
- 소요: 코드 3~4h + 자연 대기

**우선순위 2 · v2 인증 아키텍처 Phase 1 MVP** (보안 심화)
- localStorage → httpOnly 쿠키
- JWT + 24h 만료 (pyjwt 이미 설치)
- Grace period 2~4주 병행
- 소요: 12~14h
...
| 2026-07-23 | v2.0 | 7개 배포 완결 반영 (v1.38~v1.43) · P4-1/P4-6/P4-5/P2-1/P2-2/P2-1b 완료 표시 · 화약고 가설 재검토 신규 우선순위 1 · 남은 백로그 재정렬 | a870c5a |
| 2026-07-27 | v3.0 | 5개 추가 배포 완결 (v1.44~v1.48) · P2-2b/P2-2c/P2-2d 완결 · **화약고 v2 조건 확정** (F-Score 4 · 규모 필터) · **Tier 1 lock 10 종목** (경인전자 강등) · **정체성 v2.0** (투자 이익 창출) · **사용자 활용 단계** · 다음 세션은 Phase 2 준비 or 소소 개선 or 신 요구 대기 | (pending) |
