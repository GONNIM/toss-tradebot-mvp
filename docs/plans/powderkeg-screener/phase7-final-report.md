...
### 10-5. 핵심 정책 함의 · **지시서 가설 재검증** (v1.9 · 5년 backfill 후 최종)

1. **A3 담보제공 · 가설 반박** (표본 1,550 · 매우 강한 통계 유의)
   - 지시서: 오너 현금 수요 → 배당·자사주 → **매수 후보**
   - 실측: 12M **-11.67%** · win_rate 23.7% · **t=-5.42**
   - 6M · -8.19% · t=**-7.07** (최강 유의)
   - 결론: 담보제공 단독으로는 매수 시그널이 아님 · v1.5 실증 방향성 재라벨로 자동 대응 (§13)

2. **B1 횡령·배임 · 가설 완전 검증** (표본 160)
   - 지시서: **즉시 제외**
   - 실측: 12M **-17.50%** · win_rate 21% · t=-7.18 (매우 강한 통계 유의)
   - 6M · -9.35% · t=-5.06
   - 결론: §7-3 B 타입 즉시 제외 규칙 유효성 확정 · Telegram urgent 알림 정당

3. **B2 감사 비적정 · 표본 확대 후 회복 패턴 희석**
   - v1.2 (표본 57): 3M -14.5% → 12M **+16.2%** (초기 하락 후 회복)
   - v1.9 (표본 155): 6M **-11.93%** (**t=-2.60**) · 12M -1.89%
   - **함의**: 표본 확대 시 회복 패턴이 사라짐 · 극단값 상승 이벤트 희석 · **더 안정적 결론 = 6개월까지 음수 유의**

4. **B3 거래정지 · 표본 5배 확대 시 극단값 희석** (표본 1,440)
   - v1.2 (표본 317): 12M **-27.6%** · t=-11.45 (강한 음수)
   - v1.9 (표본 1,440): 12M **-0.97%** · t=-0.14 (**방향성 무의미**)
   - **핵심 관찰**: 5년 전 이벤트는 회복 후 재상장 케이스도 포함 · 최종 12M 은 편차 큼
   - **함의**: 즉시 제외는 여전히 필요 (1d -1.85% · t=-3.93 · 단기 급락) · 12M 은 회피보다 관찰
   - fallback 5거래일 후행에도 1,318건 여전히 데이터 부재 (상장폐지) · **의도된 결과**

5. **A1 오너 사법 (표본 41)** · 표본 근접 · v1.7 뉴스 크롤링으로 확대 예상
   - 1m · +1.96% (초기 반등) → 3m · -5.27% (**t=-2.32** · 유의) → 12m · +1.48%
   - **U자 회복 패턴** 후보 · 표본 확대 후 재검증 필요
...
| 2026-07-16 | v1.9 | 전문가 리뷰 반영 · **§10-3 표 수치 정합화** · 실 캐시 재작성 (A3 12M -11.67% · B3 5M valid=1440 등) · §10-2/§10-3/§10-5 전체 갱신 · 5/5 리뷰 완결 | (본 커밋) |
