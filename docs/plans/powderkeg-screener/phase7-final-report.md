...
- 프로덕션 데이터 · 118,000+ DART 매핑 · 400+ 재무 · **168 최대주주** · 44 이벤트
  (v1.19 · `POST /collectors/dart-shareholders` · 저PBR<1.5 재무 있는 117 티커 추가 수집 · 51 → 168 · 3.3배 확대 · 2026-07-16)
...
## 4. 자동 감시 · Phase 7-3 (v1.8 · 5분 스펙 준수 개편 반영)

### APScheduler 잡 (`backend/powderkeg/scheduler.py`)
- `powderkeg_events_poll` · **3분 주기** · DART 공시 폴링 (v1.8 · 30m→3m)
- `powderkeg_triggers` · **1분 주기** · Type A/B 액션 처리 (v1.8 · 5m→1m)
- `powderkeg_news_poll` · **15분 주기** · A1/A2/A6 뉴스 크롤링 (v1.7 신규)
- `powderkeg_holding_expiry` · **daily 08:00 KST** · 12개월 재평가 (v1.3 신규)
- `POWDERKEG_ENABLED=true` (기본 활성)
- **최악 지연 · 4분** (< 5분 스펙 · §16 상세)

### 이벤트 타입 처리
| 타입 | 예시 | 액션 | 알림 (v1.5 실증 방향성 반영) |
|---|---|---|---|
| **Type A · validated** | 백테스트 게이트 통과 | 3분 감지 → 1분 알림 | 🎯 [매수 후보 · VALIDATED · X] |
| **Type A · observed_negative** | 담보제공 등 · 백테스트 음수 (n≥50, 12M<-5%) | 알림에 실증 캐비어트 삽입 | 🔬 [관찰 후보 · 백테스트 음수 · X] |
| **Type A · observing** | 표본 부족 or 불명확 | 관찰 후보 알림 | 📊 [관찰 후보 · X] |
| **Type A1** | 오너 사법 리스크 | LLM classifier · 회사자금 관련성 판정 | Telegram · 상세 판정 |
| **Type B** (부정) | 횡령·감사비적정·거래정지 | 4분 이내 리스트 즉시 제거 | 🚨 urgent + DO NOT TOUCH |
...
## 6. 프로덕션 데이터 현황 (2026-07-16 · v1.11 최신 · 5년 backfill 후)
...
| 공정위 대기업집단 | **136 티커 (49 그룹)** (v1.13 · 21그룹→49그룹 확장) | 수동 seed · 2025 FTC 88개 중 51개 상장 계열사 · 나머지 37개 v2 자동 크롤러 |
| KRX 시장 스냅샷 | 2,765 | pykrx |
| PowderKegEvent 처리 (5년 backfill 후) | **~5,100** (A3=1,616 · B3=2,758 · B1=325 · B2=224 · A1=68 · A5=4) | DART 공시 + 뉴스 RSS (v1.7) |
| 백테스트 캐시 (9 event_type) | 9 (event_type + `{type}__powderkeg_passed` v1.10 층화) | PowderKegBacktestReport |
| 스크리너 run | 12+ | 수동 + 스케줄러 |
| 화약고 리스트 (승격) | 1 (035890 서희건설) | 10/10 · 부분 8~9 통과 후보 6종 |
...
| §7-3 이벤트 트리거 | B 공시 5분 내 리스트 제거 + 알림 | APScheduler 3분/1분 (v1.8 개편) · 최악 4분 · 002070·082270 텔레그램 도착 | ✅ 완결 |
| §7-4 백테스트 게이트 | 이벤트 타입별 CAR · validated 게이트 코드 강제 | 5년 backfill · CAR 1d/1m/3m/6m/12m · 표본 **A3=1,550, B1=160, B2=155, B3=1,440** (v1.11 imputed 이중 리포트) · validated 게이트 정확 작동 | ✅ **완결** (§10 상세) |
...
**종합** (v1.11 최신): 지시서 6 항목 · **전 완결** (§7-1 ~ §7-6) · 신규 편집 UI 4 항목 전 완결 · 전문가 리뷰 5/5 조치 완료 + 통계 지적 #1/#2 정정 (§10-5 · §17 imputation 방법론).
...
### 9-1. §7-4 백테스트 정밀화 (✅ **완결 · v1.11 최종 · 2026-07-16**)
- ~~CAR window 확장~~ · ✅ WINDOW_DAYS 코드 · 1d/1m/3m/6m/12m
- ~~5년 아카이브 backfill~~ · ✅ 실행 완료 · **최신 표본 A3=1,550 · B1=160 · B2=155 · B3=1,440** · §10 참조
- ~~entry_price_zero 데이터 gap~~ · ✅ `event_study.py` fallback 5 거래일 후행 검색 (`5e72174`)
- ~~다양한 이벤트 타입별 CAR 리포트~~ · ✅ A1~A6/B1~B3 9종 계산 · 유의성 검증
- ~~상폐 -100% imputation~~ · ✅ **v1.10 (`5f95330`)** · 이중 리포트 · valid vs imputed
- ~~화약고 층화 백테스트~~ · ⚠️ **코드 완결 · 표본 0 대기** · POST /backtest/stratified/{type} · 화약고 리스트 확대 후 재실행 (`5f95330`)
...
### 9-5. 이벤트 확장 (v1.7 뉴스 크롤링 완결 · 나머지 v2)
- ~~LLM 뉴스 크롤링 (§7-1-4)~~ · ✅ **v1.7 (`4b37615`)** · Sprint 2 T54 재사용 · A1/A2/A6 15분 잡
- **다양한 이벤트 타입** · Type A/B 세분화 · v2
- **오너 개인 사법 뉴스** · Anthropic Haiku 상세 판정 확장 · v2
...
**5년 backfill 최종 캐시** (2026-07-16 재캐시 · v1.11 imputation 반영):

| Type | 지시서 원 가설 | Total | Valid | Delisted (imputation 대상) | 표본 ≥ 50 |
|---|---|---|---|---|---|
| **A1** 오너 사법 | 매수 후보 | 68 | 41 | ~5 (추정) | ⚠️ (근접 · v2 뉴스 확대) |
| **A2** 상속 | 매수 후보 | 0 | 0 | 0 | ❌ (뉴스 크롤링 · v1.7 등록) |
| **A3** 담보제공 | 매수 후보 | **1,616** | **1,550** | 66 | ✅ |
| **A4** 5% 보고 | 매수 후보 | 0 | 0 | 0 | ❌ (D 타입 세부 매칭) |
| **A5** 배당/자사주 | 매수 후보 | 4 | 4 | 0 | ❌ (표본 부족) |
| **A6** 저PBR 압박 | 매수 후보 | 0 | 0 | 0 | ❌ (정책 발표 · 뉴스) |
| **B1** 횡령·배임 | 즉시 제외 | **325** | **160** | **165** | ✅ |
| **B2** 감사 비적정 | 즉시 제외 | **224** | **155** | 69 | ✅ |
| **B3** 거래정지 | 즉시 제외 | **2,758** | **1,440** | **1,318** | ✅ |

**표본 완결 4/9** · A1 은 41 로 근접 · **B1/B3 상폐 비율 매우 높음** (delisted 대량) · v1.11 imputation 필수.
...
## 17. 2차 전문가 리뷰 · Priority 1~5 완결 (v1.18~v1.21 · 2026-07-16)

### 17-1. 2차 리뷰 진단 요약

> **핵심 진단**: "1개는 시장의 답이 아니라 파이프라인의 답. 스크리너는 사실상 최대 51개 후보 중에서 1개를 뽑았다. 51개 중 1개 통과 (≈2%) 비율을 전체 저PBR 풀 (~580) 에 적용하면 12~30개."

**리뷰어 5 권장 조치 · 우선순위 순 진행**:
1. 퍼널 워터폴 UI (관측성)
2. 저PBR 유니버스 전환 (커버리지)
3. 결측 vs 탈락 분리 (분류 정확화)
4. 티어제 (사용성)
5. 역추적 캘리브레이션 (검증)

### 17-2. Priority 1 · 퍼널 워터폴 UI (`v1.18` · `8e6e816`)

- Backend · `GET /list/funnel?run_id=X`
  - universe_size · data_incomplete · evaluable · per_condition · final_passed
- Frontend · `FunnelCard` 컴포넌트
  - amber 배경 · 접기/펼치기 · 조건별 통과 바 그래프
  - 병목 조건 즉시 시각화

**성과**: "왜 1개?" 답이 UI 에 항상 있음.

### 17-3. Priority 2 · 저PBR 유니버스 전환 (`v1.19` · `5f40f51`)

- Backend · `GET /candidates/low-pbr?market=ALL`
  - market="ALL" · KOSPI+KOSDAQ 통합 · limit 최대 2000
- Frontend · `LowPbrDiscoveryCard`
  - 파라미터 조작 · PBR 상한 · 시장 · 시총 하한
  - 원클릭 대량 스크리닝

**성과**: 유니버스 전환 도구 완결. 실측 · 재무 있는 저PBR 25 종목 노출 (초기 v1.19 5f40f51).

**v1.19 후속 실행 (2026-07-16 · 3차 리뷰 §3 유니버스 확대)**:
- `POST /collectors/dart-shareholders` · 117 티커 수집 (재무 있는 저PBR<1.5)
- 응답 · `{"total":117,"collected":117,"empty":0,"failed":0}`
- **최대주주 총계 · 51 → 168** (§6 반영 · 3.3배 확대)
- 재무 백필 (~2,700 상폐 포함 추가 콜) · **P2 대기 · [`3rd-review-response.md §5`](./3rd-review-response.md#5-3-유니버스--6-팔림세스트-종결-p1) 참조**

### 17-4. Priority 4 · 티어제 (`v1.20` · `e24a650`)

- Backend · `_compute_tier` · conditions_json True count 기반
  - tier_1_passed (10/10) · tier_2_near (8~9/10) · tier_3_watch (7/10) · cash_suspect · rejected
- Frontend · `TierBadge` 컴포넌트 · 🥇🥈🥉⚠️❌
  - Tier 2/3 · 병목 조건 명시 (예: "F-Score · 감사의견")
  - 티어 필터 드롭다운
...
|---|---|---|
| 🥇 Tier 1 | 1 | 서희건설 |
| 🥈 Tier 2 (8~9) | 5 | 성우하이텍 · 골프존홀딩스 · 솔브레인홀딩스 · 휴온스글로벌 · 이지홀딩스 |
| 🥉 Tier 3 (7) | 8 | 컴투스 · 하림지주 · 유진기업 · 인탑스 · LS증권 · 엠케이전자 · CJ ENM · 동화기업 |
| ⚠️ cash_suspect | 2 | (분식 탐지) |
| ❌ rejected | 9 | |

**공통 병목** · **F-Score · Tier 2/3 · 13/13 종목 전부** · 리뷰어 지적 정확 실증.

### 17-5. Priority 5 · 역추적 캘리브레이션 (`v1.21` · 태광산업+KCC 실측)

**태광산업 (003240)** · 이 패턴의 공인된 실물 사례
- PBR 0.24 (매우 저평가 ✅)
- **순현금/시총 61.9%** (조건 2 통과 ✅ · 리뷰어 예측 반박)
- 지분율 54.5% (✅)
- **6/10 통과** · Tier 3

| 리뷰어 예측 | 실측 | 판정 |
|---|---|---|
| ⑦ 영업흑자 필터에서 탈락 | ✅ **정확 예측** · 조건 7 False | 확증 |
| ② 순현금 정의 편협 (장기금융 제외) | ❌ **예측 반박** · 61.9% 통과 | 반박 |
| ⑧ F-Score 구조적 낮음 | 부분 정합 · None (2년 데이터 부족) | 데이터 이슈 |

**KCC (002380)** · 스크리너 타깃 아님 (참고용)
- PBR 0.74 · 저PBR 아님
- 순현금 **-112%** · 부채 초과
- 지분율 35% · 임계 아래
- 3/10 rejected · 이 카테고리 종목 아님

### 17-6. Priority 3 · data_incomplete 분리 (부분 완결)

- 퍼널 카드 (`v1.18`) 에 결측 카운트 분리 표시
- v2 · PowderKegList.status 재구조화 · rejected 와 data_incomplete 완전 분리 대기

### 17-7. Priority 5 발견 기반 · v2 백로그 (신규 5건)

리뷰어 원 말: "알려진 정답이 통과 못 하는 필터는 정의가 잘못된 필터."

1. **⑦ 영업흑자 임계 완화 · v2**
   - 현재 · "3년 중 2년 흑자" · 정체형 종목 (태광산업 실증) 배제
   - 제안 · "5년 중 3년" or 사용자 조절 · 정체 허용
   - config · `POWDERKEG_OP_PROFIT_YEARS_MIN`
2. **⑧ F-Score 임계 하향 · v2**
   - 현재 · 6 (25 종목 중 통과 2 · 8%)
   - 제안 · 5 or 재무건전성 항목만 필수 (수익성 배제)
   - config · `POWDERKEG_FSCORE_MIN=5`
3. **다년 재무 자동 수집 잡 · v2**
   - audit·F-Score 병목 · 데이터 결측 원인
   - 스케줄러 잡 · 매월 · 부족 종목 자동 backfill
4. **정체형 종목 특성 필터 · v2**
   - 자산 회전율 낮은 종목 · 예외 처리
   - "매출/자산 < 0.5 이고 순현금 > 30% 이면 조건 7 완화"
5. **KCC 유사 종목 카테고리 · v2**
   - net_debt 초과 종목 별도 분류
   - "high_debt" tier · 스크리너 타깃 아님을 명시

### 17-8. 리뷰어 진단 정합성 요약

| 리뷰어 예측 | 실측 결과 | 판정 |
|---|---|---|
| 51개 중 1개 → 저PBR 확대 시 12~30개 예상 | Tier 1(1) + Tier 2(5) + Tier 3(8) = **14개 관찰 후보** | ✅ 정확 |
| F-Score 구조적 낮음 · 함정+사냥감 배제 | 25 종목 F-Score 통과율 **8%** | ✅ 정확 |
| 태광산업 · 영업흑자 탈락 예상 | 조건 7 False · **정확 실측** | ✅ 정확 |
| 태광산업 · 순현금 정의 편협 탈락 예상 | 조건 2 True · **반박** | ❌ 반박 |
| 리스트 0~1개 노이즈 · 티어제로 완화 | Tier 2 5개 + Tier 3 8개 노출 | ✅ 정확 |

**리뷰어 정확도 · 4/5 · 압도적** · 5 권장 조치 완결 후 Phase 7 스크리너 정합성 대폭 향상.
...
| 2026-07-16 | v1.11 | 리뷰어 지적 #1 **정량 확증** · §10-5 결론 정정 · B3 -0.97%→-53.45% (t=-15.49) · B1 -60.44% · B2 -32.52% · A3 -15.32% · v1.9 B3/B2 "회복 관찰" 오독 정정 · A3 층화 표본 0 확인 | `d9b742a` |
| 2026-07-16 | v1.12 | 리뷰어 지적 #3 (팔림세스트) · §4 잡 주기 3m/1m 반영 · §6 이벤트 5,100건 · §8-1 표본 최신 (A3=1,550 등) · §8-2 종합 "전 완결" · §9-1 v1.10/v1.11 반영 · §9-5 v1.7 완결 · §10-2 delisted 컬럼 추가 | `6b44270` |
| 2026-07-16 | v1.13 | 리뷰어 지적 #4 (공정위 seed 부족) · 21 그룹 → **49 그룹** · 72 티커 → **136 티커** · 2025 FTC 88개 중 51개 대응 · 잘못된 항목 (셀바스AI/NAVER 오분류) 정정 | `f071359` |
| 2026-07-16 | v1.14 | 리뷰어 지적 #5 (강건성 뱃지) · screener 조건별 임계 margin 계산 · robustness_score/grade (strong/moderate/borderline/at_risk) · 프론트 뱃지 표시 · 서희건설 at_risk 1.6% 실측 확증 | `2fe1820` |
| 2026-07-16 | v1.15 | run_id KST 시간 표기 (UTC → KST +9h · K suffix) · 프론트 fmtRunIdKst 자동 변환 | `71cf2f2` |
| 2026-07-16 | v1.16 | UX 친밀화 · OnboardingBanner · ReScreenGuide · 필터 한국어화 · 사용자 지적 대응 | `d107c60` |
| 2026-07-16 | v1.17 | 삭제 후 재평가 재출현 버그 fix · admin/list/remove 모든 run unlock · screener union 최신 run 만 · 지적 #6 게이트 문구 4조건 | `666ac4a` |
| 2026-07-16 | v1.18 | 2차 리뷰 Priority 1 · 퍼널 워터폴 UI · GET /list/funnel · FunnelCard · 조건별 통과 바 그래프 | `8e6e816` |
| 2026-07-16 | v1.19 | 2차 리뷰 Priority 2 · 저PBR 유니버스 전환 · GET /candidates/low-pbr market=ALL · LowPbrDiscoveryCard | `5f40f51` |
| 2026-07-16 | v1.20 | 2차 리뷰 Priority 4 · 티어제 · tier_1/2/3/cash_suspect/rejected · TierBadge + 병목 조건 표시 · Tier 2 5개 발굴 | `e24a650` |
| 2026-07-16 | v1.21 | 2차 리뷰 Priority 5 · 역추적 캘리브레이션 실측 · 태광산업 6/10 (⑦ 영업흑자 탈락 · ② 순현금 통과) · KCC 3/10 (타깃 아님) · §17 신설 · v2 백로그 5건 신규 | (본 커밋) |
