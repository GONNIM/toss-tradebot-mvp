...
|---|---|---|---|---|
| **A1** 오너 사법 | 매수 후보 | 10 | 6 | ❌ (v2 뉴스 크롤링) |
| **A2** 상속 | 매수 후보 | 0 | 0 | ❌ (뉴스 크롤링 필요) |
| **A3** 담보제공 | 매수 후보 | **508** | **497** | ✅ |
| **A4** 5% 보고 | 매수 후보 | 0 | 0 | ❌ (D 타입 세부 매칭) |
| **A5** 배당/자사주 | 매수 후보 | 3 | 3 | ❌ (표본 부족) |
| **A6** 저PBR 압박 | 매수 후보 | 0 | 0 | ❌ (정책 발표 · v2) |
| **B1** 횡령·배임 | 즉시 제외 | **100** | **57** | ✅ |
| **B2** 감사 비적정 | 즉시 제외 | **88** | **57** | ✅ |
| **B3** 거래정지 | 즉시 제외 | **539** | **317+** | ✅ |

**표본 완결 4/9** · 나머지 A1/A2/A4/A5/A6 는 v2 뉴스 크롤링 (§7-1-4) 대상.

### 10-3. CAR 결과 (지시서 §7-4 window 스펙 완결)

**A3 담보제공** (지시서 가설: **매수 후보**)
| Window | n | mean | win_rate | t-stat |
|---|---|---|---|---|
| 1d | 495 | -0.9% | 41.6% | **-2.91** |
| 1m | 480 | -3.0% | 41.5% | **-3.41** |
| 3m | 480 | -4.9% | 32.2% | **-3.10** |
| 6m | 477 | -10.7% | 24.9% | **-6.19** |
| 12m | 477 | **-16.4%** | 22.4% | **-6.95** |

**B1 횡령·배임** (지시서 가설: **즉시 제외**)
| Window | n | mean | win_rate | t-stat |
|---|---|---|---|---|
| 12m | ~50 | **-40.2%** | ~15% | **-11.45** |

**B2 감사 비적정** (지시서 가설: **즉시 제외**)
| Window | n | mean | win_rate | t-stat |
|---|---|---|---|---|
| 1m | 57 | +0.4% | - | - |
| 3m | 57 | -14.5% | - | **-4.43** |
...
**B3 거래정지** (지시서 가설: **즉시 제외**)
- 12m mean = **-27.6%** · win_rate 13% · t=**-11.45**
...
### 10-5. 핵심 정책 함의 · **지시서 가설 재검증**

1. **A3 담보제공 · 가설 반박**
   - 지시서: 오너 현금 수요 → 배당·자사주 → **매수 후보**
   - 실측: 12M **-16.4%** · win_rate 22% · t=-6.95 · **매우 유의미하게 음수**
   - 결론: 담보제공 단독으로는 매수 시그널이 아님 · **다른 조건과 결합 조건부 검증 필요** (예: 화약고 10 조건 통과 후 담보제공 시)

2. **B1 횡령·배임 · 가설 완전 검증**
   - 지시서: **즉시 제외**
   - 실측: 12M **-40.2%** · **정량적 정당성**
   - 결론: §7-3 B 타입 즉시 제외 규칙 유효성 확정 · Telegram urgent 알림 정당

3. **B2 감사 비적정 · 회복 패턴 발견**
   - 3M -14.5% → 12M **+16.2%** · **초기 하락 후 회복**
   - 함의: 감사 재의견 통과 케이스 회복 · v2 세부 분석 · 감사 재적정 여부로 층화 필요

4. **B3 거래정지 · 가설 검증 + entry_price 데이터 gap 잔존**
   - 12M **-27.6%** · 즉시 제외 정당
   - fallback 5거래일 후행에도 222건은 여전히 데이터 부재 (상장폐지) · **의도된 결과**
...
- **5년 backfill 후**: B3 valid=317 · fallback fix 없었으면 valid=100 이하 추정 · **fallback 로 대량 회복**
