...
### P2-4d 완결 (2026-07-18 · v1.32 · 커밋 `a59494a`)

3 종목 표기 실측 후 `_DEBT_KEYWORDS` 추가 확장:
- LG화학(051910) · `"차입금"` 단독 · 유동 7.6조 + 비유동 19.7조
- 영풍(000670)  · `"유동차입금"` / `"비유동차입금"`
- LS증권(078020) · `"III. 차입부채"` (증권업 로마 숫자 계정)

추가 키워드: `"차입금"` (substring · 유동/비유동차입금 커버) · `"차입부채"` (증권업)

**결과**:
| 종목 | total_debt (파싱 성공) | tier 변화 |
|---|---|---|
| LG화학 | 27조 3,760억 | rejected 유지 (nc=-1.04) |
| 영풍   | 6,122억      | rejected 유지 (nc=-5.93) |
| LS증권 | 4조 7,263억  | **rejected → tier_3_watch** (7/10) |

**26 종목 total_debt 매칭 100%** 달성.

### P2-4e 완결 (2026-07-18 · v1.33 · 커밋 `3b90573`)

3층 판별 로직 신설 (`order_industry_seed.py::should_apply_contract_liab_adjustment`):
1. **금융업(FINANCIAL_INDUSTRY_TICKERS · 18종) → False** · 예수부채 성격 · 조정 스킵
2. **명시 시드(ORDER_INDUSTRY_TICKERS · 18종) → True** · 건설·조선·플랜트
3. **contract_liab/market_cap > 3% → True** · 자동 판별 (임계 실측 근거: 다우데이타 10% 조정 / SK이노 0.9% 미조정)

FINANCIAL_INDUSTRY_TICKERS 신설:
- 은행 8종 (기업은행·KB금융·신한지주·하나·우리·BNK·JB·DGB)
- 증권 5종 (LS증권·삼성증권·미래에셋증권·대신·유안타)
- 보험 4종 (삼성생명·한화생명·삼성화재·DB손해보험)

**5 종목 판별 결과 실측**:
| 종목 | 판별 | nc_raw | nc_adj (판정 근거) | reject sector |
|---|---|---|---|---|
| 한화(000880) | **자동(cl>3%)** | -5.44 | **-24.32** | `자동(cl>3%)` (계약부채 121조) |
| CJ ENM(035760) | 자동(cl>3%) | -2.56 | -3.45 | `자동(cl>3%)` |
| SK이노(096770) | 미판별 (0.9%) | -1.58 | (원값) | (조정 없음) |
| 기업은행(024110) | **금융업 스킵** | -11.35 | (원값) | (조정 없음) |
| 다우데이타(032190) | 자동(cl>3%) | 6.90 | 6.80 | (cash_suspect · 조건 6 별건) |
| 서희(035890 · 참조) | 명시 시드 | 0.163 | 0.163 | `건설` |

**tier 판정 영향**: 5 종목 모두 조건 2 임계(>40%) 안 넘음 · tier 변화 없음. 그러나 **한화 nc가 -5.44 → -24.32로 극적 정확도 향상** · 향후 신규 승격 종목이 계약부채로 부풀림될 위험 원천 제거.

### 잔존 이슈 (v2 백로그)
- **예수부채 별도 로직** · 은행·증권업 예금성 부채 · 계약부채와 성격 다름 (v2)
- **raw/adj/sector UI 저장** · `PowderKegList` 컬럼 신설 검토 (조회 시 재계산 대신 저장) — Nice-to-have
- **KRX sector_code 자동 판별** · 하드코딩 시드를 sector 자동 판별로 대체 (v2)
...
| 2026-07-18 | v1.6 | P2-4c 26 종목 재파싱 완료 · debt 매칭 20→23 · contract_liab 0→5 · tier 실질 변화 서희 1건만 · 잔존 이슈 P2-4d(LG화학 등 3종목) · P2-4e(한화 등 계약부채 5종목) 백로그 신설 | `e374745` |
| 2026-07-18 | v1.7 | P2-4d 완결 · `"차입금"`·`"차입부채"` 매핑 추가 · LG화학·영풍·LS증권 파싱 성공 · LS증권 rejected→tier_3_watch · **26/26 total_debt 100% 매칭** · 남은 P2-4e 는 v2 백로그로 | `d2c3ac0` |
| 2026-07-18 | v1.8 | P2-4e 완결 · 3층 판별 (금융업 스킵 · 명시 시드 · 자동 cl>3%) · FINANCIAL_INDUSTRY_TICKERS 18종 · 한화 nc -5.44→-24.32 극적 정확도 향상 · 5/5 종목 판별 실측 확증 | (pending) |
