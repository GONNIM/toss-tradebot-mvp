...
### 3-☆ 결론 상태 5분류 (2026-09-14 · Fable · Phase C 세션 6 · 예외 없음)

리포트/STATUS 는 다음 5개 중 하나로만 결론 표기 · 자의 조합 금지:

| 상태 | 정의 | 승격/강등 규칙 |
|---|---|---|
| **확인 (confirmed)** | 사전 커밋 임계 통과 + CI 하한 > 0 + 전향 검증 통과 | 유지 · 실전 규칙 반영 |
| **잠정 확인 (tentative · 전향 검증 조건부)** | 사전 커밋 임계 통과 + CI 하한 > 0 · 단 n < 100 or 시총 부재 다수 or 견고성 미검 | 2026-11-15 전향 평가 (WP56) 재현 시 확인 승격 · 불재현 시 유보 |
| **부재 (absent)** | mean 미달 or CI 하한 ≤ 0 · 폐기 조건 발동 (deprecation_triggered=true) | 재개 조건 = 사전 등록 재실행 |
| **검증 불가 (unverifiable)** | 표본 부족 (n < 10) or 데이터 층 부재 or held_for_review 지속 | 데이터 보강 후 재실행 |
| **유보 (pending)** | 임계 미달 · CI 하한 > 0 · 방향 관측 · 폐기 미도달 | 표본 확대 후 재판정 |

**적용 원칙**:
- 상단 5줄 결론에는 이 5분류 라벨만 사용 · "성공/실패" 등 임의 표현 금지
- 잠정 확인 → 전향 평가 결과에 따라 자동 승격/강등 (STATUS 생성기가 seal + WP56 리포트 대조)
- 견고성 검증 (WP63-3 등) 통과 후에만 확인 승격 가능
- **F4 30d = 잠정 확인 (전향 검증 조건부) · F4 180d = 유보** (2026-09-14 시점)
